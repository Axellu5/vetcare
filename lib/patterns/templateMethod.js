...
import { buildFilterConditions } from "../utils/filter.js";

/**
 * Safely parses a date-only string (YYYY-MM-DD) to a Date at UTC noon.
 * Prevents timezone-related day shifts that occur with UTC midnight.
 *
...
class OwnerService extends BaseCrudService {
   ...
   * Uses buildSearchFilter utility for multi-field OR search.
   * @override
   */
  buildWhere(filters) {
    return {
      ...buildSearchFilter(filters.search, ["firstName", "lastName", "email"]),
    };
  }
...
class PetService extends BaseCrudService {
   ...
   * Uses buildFilterConditions for ownerId/species and buildSearchFilter for name.
   * @override
   */
  buildWhere(filters) {
    const searchFilter = buildSearchFilter(filters.search, ["name"]);
    const where = buildFilterConditions({ ownerId: filters.ownerId, species: filters.species });
  ...
  transformInput(data) {
    const clean = { ...data };
    if (clean.birthDate) clean.birthDate = safeParseDate(clean.birthDate);
...
class VisitService extends BaseCrudService {
   ...
   * Uses buildFilterConditions utility for all conditions.
   * @override
   */
  buildWhere(filters) {
  ...
  transformInput(data) {
    const clean = { ...data };
    if (clean.date) clean.date = safeParseDate(clean.date);
  ...
  /**
   * Deletes related VisitService records before removing the visit itself.
   * Without this, Prisma throws a foreign key constraint error.
   * @override
   */
  async beforeDelete(id) {
...
class AppointmentService extends BaseCrudService {
  ...
  /**
   * Supports filtering by vetId, ownerId, petId, status, or date range.
   * Uses buildFilterConditions utility for all conditions.
   * @override
   */
  buildWhere(filters) {
