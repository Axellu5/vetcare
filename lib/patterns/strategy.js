...
class SortByName extends SortStrategy {
  /**
   * Alphabetical sort on the `name` field using Lithuanian locale.
   *
   * Uses localeCompare with "lt" locale so that letters with diacritics
   * sort in the correct Lithuanian alphabetical position:
   *   A < Ą < B,  C < Č < D,  E < Ę < Ė < F,  I < Į < Y,
   *   S < Š < T,  U < Ų < Ū < V,  Z < Ž
   * Without an explicit locale, PostgreSQL and JS default collations place
   * all diacritic letters after Z, producing wrong cross-page orderings.
   *
   * sensitivity: "accent" — diacritics are significant (Ą ≠ A) but case
   * is ignored (a = A), which is the expected behaviour for name lists.
   *
   * Falls back to `fullName` if the object has no `name` property
   * (OwnerDTO exposes fullName instead of name).
   *
   * @param {object[]}     data  - Objects with a `name` or `fullName` string property.
   * @param {"asc"|"desc"} order - "asc" = A→Ž, "desc" = Ž→A.
   * @returns {object[]} New sorted array.
   *
   * @example
   * new SortByName().sort([{ name: "Žilvinas" }, { name: "Ąžuolas" }, { name: "Benas" }], "asc");
   * // → [{ name: "Ąžuolas" }, { name: "Benas" }, { name: "Žilvinas" }]
   */
  sort(data, order = "asc") {
    return [...data].sort((a, b) => {
      const nameA = a.name ?? a.fullName ?? "";
      const nameB = b.name ?? b.fullName ?? "";
      const cmp = nameA.localeCompare(nameB, "lt", { sensitivity: "accent" });
